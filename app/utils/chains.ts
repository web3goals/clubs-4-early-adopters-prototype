...
export function getSupportedChainConfigs(): ChainConfig[] {
  ...
  if (process.env.NEXT_PUBLIC_ZKSYNC_TESTNET_CLUB_FACTORY_CONTRACT_ADDRESS) {
    chainConfigs.push({
      chain: zkSyncTestnet,
      contractAddresses: {
        clubFactory:
          process.env.NEXT_PUBLIC_ZKSYNC_TESTNET_CLUB_FACTORY_CONTRACT_ADDRESS,
      },
    });
  }
  return chainConfigs;
...
/**
 * Return club factory contract address of specified chain if it supported, otherwise return value from default supported chain.
 */
export function chainToSupportedChainClubFactoryContractAddress(
  chain: Chain | undefined
): `0x${string}` | undefined {
  return stringToAddress(
    chainToSupportedChainConfig(chain).contractAddresses.clubFactory
  );
}
